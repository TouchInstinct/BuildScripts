...
$config_argv_key = "--config"
$ignored_argv_key = "--ignored"
$ignored_resources_key = "--ignored-resources:"
$log_file_name = "UnusedLog.txt"
...
class Unused
  ...
  def parse_arguments()
      ...
      until arguments.empty?
          ...
          # find --config file
          if item == $config_argv_key
              fileName = arguments.shift

              # add --ignored-resources to array
              File.readlines(fileName).each do |line|
                  if !line.include? $ignored_resources_key
                      resources += [line.chomp]
                  end
              end
          end

          # add --ignored files to array
          if item == $ignored_argv_key
              resources += [arguments.shift]
          end
      end
